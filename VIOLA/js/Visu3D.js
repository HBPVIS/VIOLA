...
Visu.Renderer3D = function(panel,data){
	...
	this.perspCamera = new THREE.PerspectiveCamera( 70, this.rendererW / this.rendererH, 1, 10000 );
	this.orthoCamera = new THREE.OrthographicCamera( this.rendererW / - 2, this.rendererW / 2, this.rendererH / 2, this.rendererH / - 2, 1, 10000 );
	...
	var material = [new THREE.LineBasicMaterial({color: 0xff0000, transparent:true}),
		new THREE.LineBasicMaterial({color: 0x00ff00, transparent:true}),
		new THREE.LineBasicMaterial({color: 0x0000ff, transparent:true})];
    ...
    if(this.data.dataType == "binned"){
    	markerG.vertices.push(
    		new THREE.Vector3( -this.xSize/2*this.boxSize, 0, -this.ySize/2*this.boxSize ),
    		new THREE.Vector3( -this.xSize/2*this.boxSize, 0, this.ySize/2*this.boxSize ),
    		new THREE.Vector3( this.xSize/2*this.boxSize, 0, this.ySize/2*this.boxSize ),
    		new THREE.Vector3( this.xSize/2*this.boxSize, 0, -this.ySize/2*this.boxSize ),
    		new THREE.Vector3( -this.xSize/2*this.boxSize, 0, -this.ySize/2*this.boxSize ));
    	...
    	markerG.vertices.push(
    		new THREE.Vector3( -this.xSize/2-this.boxSize/2, 0, -this.ySize/2-this.boxSize/2 ),
    		new THREE.Vector3( -this.xSize/2-this.boxSize/2, 0, this.ySize/2+this.boxSize/2 ),
    		new THREE.Vector3( this.xSize/2+this.boxSize/2, 0, this.ySize/2+this.boxSize/2 ),
    		new THREE.Vector3( this.xSize/2+this.boxSize/2, 0, -this.ySize/2-this.boxSize/2 ),
    		new THREE.Vector3( -this.xSize/2-this.boxSize/2, 0, -this.ySize/2-this.boxSize/2 ));
	...
	//var boxGeo = new THREE.BoxGeometry( this.boxSize, this.boxSize, this.boxSize );
    var boxGeo = new THREE.BufferGeometry().fromGeometry(new THREE.BoxGeometry( this.boxSize, this.boxSize, this.boxSize ));
	...
	for(k = 0 ; k < this.data.nLayers ; k++){
        ...
        //3D Timeline objects
        this.timelines[k] = new THREE.MarchingCubes(this.data.xNeurons, this.data.yNeurons, this.zTimeSize, 
            new THREE.MeshPhongMaterial( { color:data.layerColors[k] ,specular:0x606060,transparent:true,blending:THREE.NormalBlending}), true, true);
        this.timelineScene.add(this.timelines[k]);
        this.timelines[k].scale.set(scale,scale,scale);//105 ~= 100/0.95 where 0.95 ~= (40-2)/40
		...
		this.yLayerPos[k] = (this.lNum-1)*this.boxSize*this.layerDisplacement/2 - this.lPos[k]*this.boxSize*this.layerDisplacement;
        ...
        if(this.data.dataType == "binned"){
    		for(i = 0; i < this.xSize; i++) {
    		    for(j = 0; j < this.ySize; j++) {
    				...
    				this.voxels[k][voxindex] = new THREE.Mesh(boxGeo , new THREE.MeshLambertMaterial({color:this.data.layerColors[k],opacity:1, transparent: false}));
    				this.voxels[k][voxindex].position.x = semibox - xsemibox + i*this.boxSize;
    				this.voxels[k][voxindex].position.y = 0;
    				this.voxels[k][voxindex].position.z = semibox - ysemibox + j*this.boxSize;
            ...
            for(i=0;i<this.xSize;i++){
                this.sumSquares[k][i] = new THREE.Mesh(squareGeo, new THREE.MeshLambertMaterial({color:this.data.layerColors[k]}));
                this.sumSquares[k][i].position.x = semibox - xsemibox + i*this.boxSize;
                this.sumSquares[k][i].position.y = 0;
                this.sumSquares[k][i].position.z = ysumpos;
                this.sumSquares[k][i].lookAt(new THREE.Vector3(this.sumSquares[k][i].position.x,0,0));
            ...
            for(i=this.xSize;i<this.xSize+this.ySize;i++){
                ...
                this.sumSquares[k][i].position.y = 0;
                this.sumSquares[k][i].position.z = semibox - ysemibox + (i-this.xSize)*this.boxSize;
                this.sumSquares[k][i].lookAt(new THREE.Vector3(0,0,this.sumSquares[k][i].position.z));
                this.oSumSquares[k].add(this.sumSquares[k][i]);
            }
            this.oVoxels[k].add(this.oSumSquares[k]);
        }
        
		this.lMarkers[k] = new THREE.Line(markerG, new THREE.LineBasicMaterial({color:this.data.layerColors[k]}));
    ...
    this.lfpMaterial = new THREE.MeshBasicMaterial({map:this.lfpTexture,side:THREE.DoubleSide,transparent:true,opacity:0.5,depthWrite:false});
    this.lfpMesh = new THREE.Mesh(new THREE.PlaneBufferGeometry(this.lfpXSize,this.lfpYSize),this.lfpMaterial);
    this.lfpMesh.position.y = this.yLayerPos[this.data.nLayers -1] - this.lfpSpacing;
    this.lfpMesh.lookAt(this.origin);
    this.lfpMesh.rotateZ(-Math.PI/2);
    if(this.data.dataType == "binned")
        this.layersScene.add(this.lfpMesh);
    
    this.lfpPosition = [this.data.layerNames.indexOf("L5E"),this.data.layerNames.indexOf("L5I")];
    this.lfpBottom = true;
    ...
    if(this.data.dataType == "binned"){
        ...
        this.timelineIndex = new THREE.Mesh(new THREE.PlaneBufferGeometry(2,2),
    	//new THREE.MeshLambertMaterial({color:0x404040, transparent:true, opacity:0.5, side:THREE.DoubleSide}));
        //this.lfpMaterial);
        ...
        this.timelineBoundingBox = new THREE.Mesh(new THREE.BufferGeometry().fromGeometry(new THREE.BoxGeometry( 1, 1, 1 )),
        ...
        geo.vertices.push(new THREE.Vector3(-1,0,0), new THREE.Vector3(1,0,0),
            new THREE.Vector3(-1,0,-0.02), new THREE.Vector3(-1,0,0.02),
            new THREE.Vector3(0,0,-0.02), new THREE.Vector3(0,0,0.02),
            new THREE.Vector3(1,0,-0.02), new THREE.Vector3(1,0,0.02));
        var linematerial = new THREE.LineBasicMaterial({color:0x000000,lineWidth:4,depthTest:false});
        ...
        this.timelineScaleX.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2);
        ...
        this.timelineScaleY.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2);
        ...
        this.timelineScaleT.scale.set(100,100,100*(this.zTimeSize-2)/(this.data.xNeurons-2));
        ...
        this.zeroMaterial = new THREE.SpriteMaterial({map:this.zeroTexture,depthTest:false});
        this.zeroctx.font = "20px serif";
        this.zeroctx.fillStyle="black";
        this.zeroctx.textBaseline = "middle";
        this.zeroctx.fillText("0", 10-this.zeroctx.measureText("0").width/2, 10);
        ...
        this.maxXMaterial = new THREE.SpriteMaterial({map:this.maxXTexture,depthTest:false});
        this.maxXctx.font = "20px serif";
        this.maxXctx.fillStyle="black";
        this.maxXctx.textBaseline = "middle";
        this.maxXctx.fillText(this.data.xSize/2, 10-this.maxXctx.measureText(this.data.xSize/2).width/2, 10);
        ...
        this.minXMaterial = new THREE.SpriteMaterial({map:this.minXTexture,depthTest:false});
        this.minXctx.font = "20px serif";
        this.minXctx.fillStyle="black";
        this.minXctx.textBaseline = "middle";
        this.minXctx.fillText(-this.data.xSize/2, 10-this.minXctx.measureText(-this.data.xSize/2).width/2, 10);
        ...
        this.maxYMaterial = new THREE.SpriteMaterial({map:this.maxYTexture,depthTest:false});
        this.maxYctx.font = "20px serif";
        this.maxYctx.fillStyle="black";
        this.maxYctx.textBaseline = "middle";
        this.maxYctx.fillText(this.data.ySize/2, 10-this.maxYctx.measureText(this.data.ySize/2).width/2, 10);
        ...
        this.minYMaterial = new THREE.SpriteMaterial({map:this.minYTexture,depthTest:false});
        this.minYctx.font = "20px serif";
        this.minYctx.fillStyle="black";
        this.minYctx.textBaseline = "middle";
        this.minYctx.fillText(-this.data.ySize/2, 10-this.minYctx.measureText(-this.data.ySize/2).width/2, 10);
        ...
        this.xMaterial = new THREE.SpriteMaterial({map:this.xTexture,depthTest:false});
        this.xctx.font = "20px serif";
        this.xctx.fillStyle="black";
        this.xctx.textBaseline = "middle";
        this.xctx.fillText("x(mm)", 30-this.xctx.measureText("x(mm)").width/2, 30);
        ...
        this.yMaterial = new THREE.SpriteMaterial({map:this.yTexture,depthTest:false,rotation:Math.PI/2});
        this.yctx.font = "20px serif";
        this.yctx.fillStyle="black";
        this.yctx.textBaseline = "middle";
        this.yctx.fillText("y(mm)", 30-this.yctx.measureText("y(mm)").width/2, 30);
        ...
        this.tMaterial = new THREE.SpriteMaterial({map:this.tTexture,depthTest:false});
        this.tctx.font = "20px serif";
        this.tctx.fillStyle="black";
        this.tctx.textBaseline = "middle";
        this.tctx.fillText("t(ms)", 50-this.tctx.measureText("t(ms)").width/2, 50);
        ...
        this.tm40Material = new THREE.SpriteMaterial({map:this.tm40Texture,depthTest:false});
        this.tm40ctx.font = "20px serif";
        this.tm40ctx.fillStyle="black";
        this.tm40ctx.textBaseline = "middle";
        this.tm40ctx.fillText(Math.round(-this.zTimeSize*this.data.resolution*0.2), 30-this.tm40ctx.measureText(Math.round(-this.zTimeSize*this.data.resolution*0.2)).width/2, 30);
        ...
        this.tm80Material = new THREE.SpriteMaterial({map:this.tm80Texture,depthTest:false});
        this.tm80ctx.font = "20px serif";
        this.tm80ctx.fillStyle="black";
        this.tm80ctx.textBaseline = "middle";
        this.tm80ctx.fillText(Math.round(-this.zTimeSize*this.data.resolution*0.4), 30-this.tm80ctx.measureText(Math.round(-this.zTimeSize*this.data.resolution*0.4)).width/2, 30);
        ...
        this.t40Material = new THREE.SpriteMaterial({map:this.t40Texture,depthTest:false});
        this.t40ctx.font = "20px serif";
        this.t40ctx.fillStyle="black";
        this.t40ctx.textBaseline = "middle";
        this.t40ctx.fillText(Math.round(this.zTimeSize*this.data.resolution*0.2), 30-this.t40ctx.measureText(Math.round(this.zTimeSize*this.data.resolution*0.2)).width/2, 30);
        ...
        this.t80Material = new THREE.SpriteMaterial({map:this.t80Texture,depthTest:false});
        this.t80ctx.font = "20px serif";
        this.t80ctx.fillStyle="black";
        this.t80ctx.textBaseline = "middle";
        this.t80ctx.fillText(Math.round(this.zTimeSize*this.data.resolution*0.4), 30-this.t80ctx.measureText(Math.round(this.zTimeSize*this.data.resolution*0.4)).width/2, 30);
        ...
        this.xzeroSprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-20;
        ...
        this.xMinSprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-20;
        ...
        this.xMaxSprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-20;
        ...
        this.xSprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-45;
        ...
        this.yzeroSprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-20;
        ...
        this.yMinSprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-20;
        ...
        this.yMaxSprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-20;
        ...
        this.ySprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)-45;
        ...
        this.tm40Sprite.position.z = 100*(this.zTimeSize-2)/(this.data.xNeurons-2)*0.4;
        ...
        this.tm80Sprite.position.z = 100*(this.zTimeSize-2)/(this.data.xNeurons-2)*0.8;
        ...
        this.t40Sprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)*0.4;
        ...
        this.t80Sprite.position.z = -100*(this.zTimeSize-2)/(this.data.xNeurons-2)*0.8;
        this.timelineScene.add(this.t80Sprite);
        
    	//Plane for sum background
        this.basePlaneHeight = this.yLayerPos[0]-this.yLayerPos[this.data.nLayers-1];
    
    	this.plane = new THREE.Mesh(new THREE.PlaneBufferGeometry((2*this.boxSize+2)*this.ySize,this.basePlaneHeight),
    	...
    	this.plane2 = new THREE.Mesh(new THREE.PlaneBufferGeometry((2*this.boxSize+2)*this.xSize,this.basePlaneHeight),
    	new THREE.MeshLambertMaterial({color:0x404040}));
	...
	var cameraLight = new THREE.DirectionalLight( 0xffffff, 0.5 ); //not fixed to camera anymore
	...
	this.camera.position.x = Math.floor(Math.cos( this.cameraAngleHorizontal * Math.PI / 180 ) * Math.cos( this.cameraAngleVertical * Math.PI / 180 ) * this.cameraDistance);
	this.camera.position.z = Math.floor(Math.sin( this.cameraAngleHorizontal * Math.PI / 180 ) * Math.cos( this.cameraAngleVertical * Math.PI / 180 ) * this.cameraDistance);
	this.camera.position.y = Math.floor(Math.sin( this.cameraAngleVertical * Math.PI / 180 ) * this.cameraDistance);
	
	this.camera.lookAt(this.origin);
	
	this.cameraDirection = new THREE.Vector3(-this.camera.position.x,-this.camera.position.y,-this.camera.position.z).normalize();
	this.cameraLateralDirection = new THREE.Vector3(-this.camera.position.x,-this.camera.position.y,-this.camera.position.z).applyAxisAngle(new THREE.Vector3(0,-1,0),Math.PI/2).normalize();
    ...
    +   '<li>'
    +       '<div>Global <input type="number" value="4" style="width:40px" onchange="r3D.setLayerSpacing(this.value)"></div>'
    +   '</li>'
    +   '<li>'
    +       '<div>LFP <input type="number" value="2" style="width:40px" onchange="r3D.setLFPSpacing(this.value)"></div>'
    +   '</li>'
    +   '<li class="spacer"></li>'
    +   '<li>'
    +       '<div>Group <input id="spacingInput" type="number" value="1" style="width:40px" onchange="r3D.setGroupSpacing(this.value)"></div>'
    +   '</li>'
    +   '<li>'
    +       '<div><input type="checkbox" checked onchange="r3D.toggleLayerGrouping(this)">Group E-I Layers</div>'
    +   '</li>'
    + '</ul>';
    document.getElementById('3DViewSubMenu').appendChild(this.layerMenuItems[0]);
    ...
    +                           '<li>'
    +                                '<div><input type="checkbox" onchange="r3D.toggleOpacity(this.checked)">Opacity</div>'
    +                            '</li>'
    +                            '<li class="spacer"></li>'
    +                            '<li>'
    +                                '<div><input type="checkbox" checked onchange="r3D.toggleSize(this.checked)">Size</div>'
    +                            '</li>'
    +                            '<li>'
    +                                '<div><input type="checkbox" disabled id="hideCheck" checked onchange="r3D.toggleHideNeurons(this.checked)">Hide inactive bins</div>'
    +                            '</li>'
    +                            '<li class="spacer"></li>'
    +                            '<li>'
    +                                '<div><input type="checkbox" onchange="r3D.togglePosition(this.checked)">Position</div>'
    +                            '</li>'
    +                            '<li>'
    +                                '<div><input id="displacementInput" disabled type="number" value="1" style="width:40px" onchange="r3D.setVerticalDisplacement(this.value)"> Vertical displacement</div>'
    +                            '</li>'
    +                        '</ul>';
    document.getElementById('3DViewSubMenu').appendChild(this.layerMenuItems[1]);
    ...
    + '<ul style="width:200px">'
    + '<li>'
    +     '<div>Opacity <input type="checkbox" checked onchange="r3D.toggleLFPOpacity()"><input id="lfpopacityslider" type="range" min="0" max="10" step="1" value="5" oninput="r3D.setLFPOpacity(this.value)" style="width:100px;vertical-align:middle;display:inline"></div>'
    + '</li>'
    + '</ul>';
    document.getElementById('3DViewSubMenu').appendChild(this.layerMenuItems[2]);
...
Visu.Renderer3D.prototype = {
    ...
    setDelay: function(v,i){
        ...
        document.getElementById('delayspan'+i).innerHTML = val;
        updated=true;
    },
    ...
    setTimeWindow: function(v,w,index){
        ...
        this.timelineBoundingBox.scale.set(5*this.xSize,5*this.ySize,200*zscale);
        this.timelineScaleT.scale.set(100,100,100*zscale);
        this.tm40ctx.clearRect(0,0,60,60);
        this.tm40ctx.fillText(-Math.round(this.zTimeSize*this.data.resolution*0.2), 30-this.t40ctx.measureText(-Math.round(this.zTimeSize*this.data.resolution*0.2)).width/2, 30);
        this.tm40Texture.needsUpdate = true;
        this.tm40Sprite.position.z = 100*zscale*0.4;
        this.tm80ctx.clearRect(0,0,60,60);
        this.tm80ctx.fillText(-Math.round(this.zTimeSize*this.data.resolution*0.4), 30-this.t80ctx.measureText(-Math.round(this.zTimeSize*this.data.resolution*0.4)).width/2, 30);
        this.tm80Texture.needsUpdate = true;
    ...
    createNeuronGeometries: function(k){
        ...
        for(i = 0; i < this.data.neuronLayers[k].length; i++){
            ...
            this.voxels[nIndex] = new THREE.Mesh(boxGeo , new THREE.MeshLambertMaterial({color:this.data.layerColors[k],opacity:1, transparent: false}));
			this.voxels[nIndex].position.x = this.data.neuronPos[nIndex][0]*this.mmToPixel;
            this.voxels[nIndex].position.y = 0;
			this.voxels[nIndex].position.z = this.data.neuronPos[nIndex][1]*this.mmToPixel;
	...
	adjustDisplayedNeurons: function(){
		...
		for(var k = 0 ; k < this.data.nLayers ; k++){
			...
			var markerG = new THREE.Geometry();
			markerG.vertices.push(
				new THREE.Vector3( (this.xMin-this.xSize/2)*this.boxSize, 0, (this.yMin-this.ySize/2)*this.boxSize ),
				new THREE.Vector3( (this.xMin-this.xSize/2)*this.boxSize, 0, (this.yMax-this.ySize/2)*this.boxSize ),
				new THREE.Vector3( (this.xMax-this.xSize/2)*this.boxSize, 0, (this.yMax-this.ySize/2)*this.boxSize ),
				new THREE.Vector3( (this.xMax-this.xSize/2)*this.boxSize, 0, (this.yMin-this.ySize/2)*this.boxSize ),
				new THREE.Vector3( (this.xMin-this.xSize/2)*this.boxSize, 0, (this.yMin-this.ySize/2)*this.boxSize ));
			this.lMarkers[k] = new THREE.Line(markerG, new THREE.LineBasicMaterial({color:this.data.layerColors[k]}));
			this.oVoxels[k].add(this.lMarkers[k]);
    ...
    toggleLFPOpacity: function(){
        if(this.lfpMesh.material.transparent == true){
            ...
            document.getElementById('lfpopacityslider').disabled = true;
        }else{
            this.lfpMesh.material.transparent = true;
            this.lfpMesh.material.depthWrite = false;
            document.getElementById('lfpopacityslider').disabled = false;
        }
        updated=true;
    },
	...
	toggleLayerGrouping: function (p){
		...
		this.updateLayerPositions();
		updated=true;
		document.getElementById("spacingInput").disabled = !p.checked;
	},
    ...
    toggleScene: function(v){
        ...
        if(v == "layers"){
            ...
            document.getElementById("layersParams").className="";
            document.getElementById("timelineParams").className="hiddenPanel";
            document.getElementById("radioscenet").checked=false;
            document.getElementById("radioscenel").checked=true;
            
            for(i = 0 ; i < this.timelineMenuItems.length ; i++){
                if(this.timelineMenuItems[i].parentNode != undefined){
                    this.timelineMenuItems[i].parentNode.removeChild(this.timelineMenuItems[i]);
                }
            }
            for(i = 0 ; i < this.layerMenuItems.length ; i++){
                document.getElementById('3DViewSubMenu').appendChild(this.layerMenuItems[i]);
            }
        }else{
            this.timelineScene.visible = true;
            this.layersScene.visible = false;
            document.getElementById("timelineParams").className="";
            document.getElementById("layersParams").className="hiddenPanel";
            document.getElementById("radioscenel").checked=false;
            document.getElementById("radioscenet").checked=true;
            ...
            for(i = 0 ; i < this.timelineMenuItems.length ; i++){
                document.getElementById('3DViewSubMenu').appendChild(this.timelineMenuItems[i]);
            }
    ...
    renderOverlay: function(){
		...
		this.overlay.fillText(this.time+"/"+this.data.simulationLength+" ms", 10, 30);
		...
		if(this.showNames && this.visu == "layers"){
			...
			for(var i = 0 ; i < this.data.nLayers ; i++){
                if(this.displayPop[i]){
    				...
    				this.overlay.fillRect(0,i*this.svgH/this.data.nLayers,20,this.svgH/this.data.nLayers);
    				this.overlay.fillText(this.data.layerNames[i], 10, this.rendererH - 10 - (this.data.nLayers - i - 1)*40);
    				this.overlay.strokeText(this.data.layerNames[i], 10, this.rendererH - 10 - (this.data.nLayers - i - 1)*40);
                }
			}
		}
    },
    ...
    moveCameraFree: function(){
        if(this.keysDown[this.keys.forward]){
            this.camera.translateOnAxis(this.forwardVector,-this.cameraTranslation*this.delta);
            updated = true;
        }
        ...
        if(this.keysDown[this.keys.straf_left]){
            this.camera.translateOnAxis(this.rightVector,-this.cameraTranslation*this.delta);
            updated = true;
        }
        ...
        if(this.keysDown[this.keys.rot_up]){
			this.cameraDirection.applyAxisAngle(this.cameraLateralDirection,this.cameraRotation*this.delta);
			this.camera.lookAt(new THREE.Vector3().addVectors(this.camera.position,this.cameraDirection));
            updated = true;
        }
        ...
        if(this.keysDown[this.keys.rot_left]){
			this.cameraLateralDirection.applyAxisAngle(this.upVector,this.cameraRotation*this.delta);
			this.cameraDirection.applyAxisAngle(this.upVector,this.cameraRotation*this.delta);
			this.camera.lookAt(new THREE.Vector3().addVectors(this.camera.position,this.cameraDirection));
            updated = true;
        }
        ...
        if(this.keysDown[this.keys.zoom_in]){
    		...
    		if(this.cameraName == "persp"){
    			this.camera.fov = this.minFov + (this.maxFov - this.minFov)*this.cameraZoom/100;
    		}else{
        ...
        if(this.keysDown[this.keys.up]){
			this.camera.translateOnAxis(this.upVector,this.cameraTranslation*this.delta);
            updated = true;
        }
    ...
    moveCameraFixed: function(){
        if(this.keysDown[this.keys.forward]){
            this.cameraAngleVertical += this.cameraRotation*180/Math.PI*this.delta;
        ...
        if(this.keysDown[this.keys.straf_left]){
			this.cameraAngleHorizontal += this.cameraRotation*180/Math.PI*this.delta;
            this.updateCameraPosition();
            updated = true;
        }
        if(this.keysDown[this.keys.straf_right]){
	...
	setupFreeCamera: function(){
        ...
        if(x == 0 && z == 0){
            this.cameraLateralDirection = new THREE.Vector3(0,0,1).normalize();
        }else{
            this.cameraLateralDirection = new THREE.Vector3(x,0,z).applyAxisAngle(new THREE.Vector3(0,-1,0),Math.PI/2).normalize();
        }    
    },
	...
	setupFixedCamera: function(){
		...
		this.cameraAngleHorizontal = Math.round(this.rightVector.angleTo(this.camera.position.clone().setY(0))*180/Math.PI);
		if(this.camera.position.z < 0)
			this.cameraAngleHorizontal = -this.cameraAngleHorizontal;
		this.cameraAngleVertical = 90 - Math.round(this.camera.position.angleTo(new THREE.Vector3(0,1,0))*180/Math.PI);
	},
    ...
    colorLFP: function(v){
        var value = v/this.data.lfpWidth+1;//range from "0 to 2" if all data inside [-lfpWidth,+lfpWidth]
        if(value < 0)
            value = 0;
        if(value > 2)
            value = 2;
        if(value < 1){
            return "rgb(255,"+Math.floor(value*255)+","+Math.floor(value*255)+")";
        }else{
            return "rgb("+Math.floor(255*(1-(value-1)))+","+Math.floor(255*(1-(value-1)))+",255)";
        }
    },
	...
	updatePlane: function (index,k){
		...
		if(this.displayPop[k]){
            ...
            if(this.data.dataType == "binned"){
    			...
    			for(var i = 0; i < this.xSize; i++) {
    				for(var j = 0; j < this.ySize; j++) {
                        logData = Math.log(1+this.data.getScaledValue(currData[i*this.ySize+j]))/logMax;
    					rData = this.data.getScaledValue(currData[i*this.ySize+j])/this.data.maxSpikesAmount;
					
    					if(rData > 0){
    						this.voxels[k][j+i*this.ySize].visible=true;
    						if(this.useSize){
    							this.voxels[k][j+i*this.ySize].scale.set(rData,rData,rData);
    						}
    						if(this.usePosition){
    							if(this.logPos){
    								if(this.groupLayers)
    									this.voxels[k][j+i*this.ySize].position.y = logData*this.vDisplacement*this.boxSize*(1 + (-k%2)*2);
    								else
    									this.voxels[k][j+i*this.ySize].position.y = logData*this.vDisplacement*this.boxSize;
    							}else{
    								if(this.groupLayers)
    									this.voxels[k][j+i*this.ySize].position.y = rData*this.vDisplacement*this.boxSize*(1 + (-k%2)*2);
    								else
    									this.voxels[k][j+i*this.ySize].position.y = rData*this.vDisplacement*this.boxSize;
    							}
    						}
    						if(this.useOpacity){
	...
	updateGLColor: function (c){
        if(this.data.dataType == "binned"){
    		...
    		for(var i = 0; i < limit; i++) {
    			for(var j = 0; j < this.ySize; j++) {
    				this.voxels[c][i].material.color = new THREE.Color(this.data.layerColors[c]);
    			}
    		}
        
            for(var i=0;i<this.xSize+this.ySize;i++){
                this.sumSquares[c][i].material.color = new THREE.Color(this.data.layerColors[c]);
            }
            

        
            this.timelines[c].material.color = new THREE.Color(this.data.layerColors[c]);
        }else if(this.data.dataType == "neuron"){
            var indices = this.data.neuronLayers[c];
            for(i = 0; i < indices.length; i++){
                if(this.voxels[indices[i]] !== undefined)
                    this.voxels[indices[i]].material.color = new THREE.Color(this.data.layerColors[c]);
            }
        }
    ...
    updateTimelineIndex: function(index){
        ...
        this.timelineIndex.position.z = Math.floor(index - this.timeOffset - this.zTimeSize/2)/this.zTimeSize*2*100*(this.zTimeSize-2)/(this.data.xNeurons-2);
        /* (index - this.timeOffset - this.zTimeSize/2) goes from -20 to 20 and we scaled the geometry by 100, while the vertices
	...
	updateLayerPositions: function () {
		...
		for(var k=0; k<data.nLayers; k++){
			if(this.displayPop[k]){
				if(this.groupLayers){
					this.yLayerPos[k] = Math.floor((this.lNum-1)/2)*this.boxSize*this.layerDisplacement/2
					+ Math.floor(this.lNum/2)*this.boxSize*this.groupDisplacement/2 
					- this.boxSize*this.groupDisplacement*Math.ceil(this.lPos[k]/2)
					- this.boxSize*this.layerDisplacement*Math.floor(this.lPos[k]/2);
				}else{
					this.yLayerPos[k] = (this.lNum-1)*this.boxSize*this.layerDisplacement/2 - this.lPos[k]*this.boxSize*this.layerDisplacement;
				}
				this.oVoxels[k].position.y = this.yLayerPos[k];
        ...
        if(!this.lfpBottom){
            this.lfpMesh.position.y = this.yLayerPos[this.lfpPosition[0]] + (this.yLayerPos[this.lfpPosition[1]] - this.yLayerPos[this.lfpPosition[0]])/2;
        }
	...
	togglePop: function (b,v){
        if(b != this.displayPop[v]){
            ...
            if(this.displayPop[this.lfpPosition[0]] && this.displayPop[this.lfpPosition[1]]){
                this.lfpBottom = false;
            }else{
                this.lfpBottom = true;
            }
    ...
    updateSumBackground: function(){
        ...
        if(size == 0)
            size = this.boxSize*2;
        this.plane.scale.set(1,this.backgroundScale*(size)/this.basePlaneHeight,1);
        this.plane2.scale.set(1,this.backgroundScale*(size)/this.basePlaneHeight,1);
    },
}