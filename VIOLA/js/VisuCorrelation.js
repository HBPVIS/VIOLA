...
Visu.Correlation.prototype = {
  ...
  computeData: function() {
    ...
    //Compute new image
    var line = [],
      temp_line, effective_bins;
    for (i = 0; i < this.maxDistance; i++) {
      for (j = 0, effective_bins = 0; j < this.average_bins_num; j++) {
        temp_line = this.data.computeCrossCorrelation(
          this.layer1, average_bins_x[j], average_bins_y[j], this.layer2,
          (average_bins_x[j] + i) % this.data.xNeurons, average_bins_y[j],
          this.maxDelay);
  ...
  drawLegends: function() {
    ...
    this.ctx.fillText(0,
                      this.paddingW - this.ctx.measureText(0).width - 2,
                      this.paddingH);
    this.ctx.fillText(sc[0],
                      this.paddingW - this.ctx.measureText(sc[0]).width - 2,
                      this.paddingH + this.displayH / 4);
    this.ctx.fillText(sc[1],
                      this.paddingW - this.ctx.measureText(sc[1]).width - 2,
                      this.paddingH + this.displayH / 2);
    this.ctx.fillText(sc[2],
                      this.paddingW - this.ctx.measureText(sc[2]).width - 2,
                      this.paddingH + 3 * this.displayH / 4);
    this.ctx.fillText(sc[3],
                      this.paddingW - this.ctx.measureText(sc[3]).width - 2,
                      this.paddingH + this.displayH);
    this.ctx.save();
    this.ctx.rotate(-Math.PI / 2);
    this.ctx.translate(-this.canvasH, 0);
    this.ctx.textBaseline = "top";
    this.ctx.fillText("Distance (mm)",
                      this.canvasH / 2 -
                        this.ctx.measureText("Distance (mm)").width / 2,
                      0);
    this.ctx.restore();

    //Bottom
    this.ctx.textBaseline = "bottom";
    this.ctx.fillText("Delay (ms)",
                      this.canvasW / 2 -
                        this.ctx.measureText("Delay (ms)").width / 2,
                      this.canvasH - 10);
    this.ctx.textBaseline = "top";
    var delay = this.maxDelay * this.data.resolution;
    var sc = [-delay, -delay / 2,
              delay / 2,
              delay];

    this.ctx.fillText(sc[0].toPrecision(2),
                      this.paddingW -
                        this.ctx.measureText(sc[0].toPrecision(2)).width / 2,
                      this.paddingH + this.displayH + 5);
    this.ctx.fillText(sc[1].toPrecision(2),
                      this.paddingW + this.displayW / 4 -
                        this.ctx.measureText(sc[1].toPrecision(2)).width / 2,
                      this.paddingH + this.displayH + 5);
    this.ctx.fillText(0,
                      this.canvasW / 2 - this.ctx.measureText(0).width / 2,
                      this.paddingH + this.displayH + 5);
    this.ctx.fillText(sc[2].toPrecision(2),
                      this.paddingW + 3 * this.displayW / 4 -
                        this.ctx.measureText(sc[2].toPrecision(2)).width / 2,
                      this.paddingH + this.displayH + 5);
    this.ctx.fillText(sc[3].toPrecision(2),
                      this.paddingW + this.displayW -
                        this.ctx.measureText(sc[3].toPrecision(2)).width / 2,
                      this.paddingH + this.displayH + 5);
    ...
    this.ctx.fillRect(this.paddingW + this.displayW,
                      this.paddingH,
                      10,
                      this.displayH);
    ...
    this.ctx.fillStyle = "black";
    this.ctx.textBaseline = "middle";
    this.ctx.fillText(this.maxVal,
                      this.displayW + this.paddingW + 20 -
                        this.ctx.measureText(this.maxVal).width / 2,
                      this.paddingH);
    this.ctx.fillText(this.minVal,
                      this.displayW + this.paddingW + 20 -
                        this.ctx.measureText(this.minVal).width / 2,
                      this.paddingH + this.displayH);
    this.ctx.save();
    this.ctx.rotate(-Math.PI / 2);
    this.ctx.translate(-this.canvasH, 0);
    this.ctx.fillText("Correlation value",
                      this.canvasH / 2 -
                        this.ctx.measureText("Correlation value").width / 2,
                      this.canvasW - this.paddingW / 2);
    this.ctx.restore();
  },
