...
Visu.Data = function(params){
    ...
    if(this.dataType == "binned"){
        ...
        this.posReady = []; //Booleans to check if positions of neurons have been loaded
        this.neuronPos = []; //Array with neurons and their positions
        this.neuronLayers = []; //Array with indices of neurons belonging in the layers;
        this.neuronSpikes = [];
...
Visu.Data.prototype = {
	...
	computeCrossCorrelation: function(l1,x1,y1,l2,x2,y2,max_delay) {
		var s=[],m1=0,m2=0,d1=0,d2=0,denom;
		...
		for(var i=0; i<this.timestamps; i++){
			d1 += (this.datasets[l1][i][x1*this.yNeurons+y1] - m1) * (this.datasets[l1][i][x1*this.yNeurons+y1] - m1);
			d2 += (this.datasets[l2][i][x2*this.yNeurons+y2] - m2) * (this.datasets[l2][i][x2*this.yNeurons+y2] - m2);
		}
		...
		for(var delay=-max_delay,index=0; delay<max_delay; delay++,index++){
			...
			for(var i=0,j=0; i<this.timestamps; i++){
				...
				j %= this.timestamps;
				s[index] += (this.datasets[l1][i][x1*this.yNeurons+y1] - m1) * (this.datasets[l2][j][x2*this.yNeurons+y2] - m2);
			}
			s[index] /= denom;
            if(s[index] < 0){
                s[index] = 0;
