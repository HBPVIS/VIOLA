...
//------------------------------------------------------------------------------
//All those functions serve to generate the parameters inputs according to the
//settings the user specified
//------------------------------------------------------------------------------
function generateUploadPanel() {

  var ih =
    '<table id="filesTable"><caption style="color:black;text-align:center">' +
       'Files to upload</caption><tr><th>Animation Data</th></tr>';

  for (var i = 0; i < data.nLayers; i++) {
    ih += '<tr><td id="hn' + data.layerNames[i] + '"><span>' + data.layerNames[
        i] + '</span><br/><span class="fileName" id="sn' + data.layerNames[i] +
      '"></span></td>' + '</tr>';

    if (data.dataType == "neuron") {
      ih += '<tr><td id="pos' + data.layerNames[i] + '"><span>' +
        data.layerNames[i] + ' Positions</span><br/>' +
        '<span class="fileName" id="posn' +
        data.layerNames[i] + '"></span></td></tr>';
    };
  };
  if (data.dataType == "binned") {
    ih +=
      '<tr><td id="lfp"><span>LFP</span><br/><span class="fileName"' +
      'id="lfpn"></span></td></tr>';
  };
  ih += '</table>';
...
function generateTimelineParams() {
  ...
  ih += '</ul>';
  ih +=
    '<span>Isolation (spikes per s) : </span><input type="range" min="1" ' +
    'max="1000" value="100" step="1" onmousedown="r3D.disableDrag(true)" ' +
    'onmouseup="r3D.disableDrag(false)" ' +
    'oninput="r3D.setIsolation(this.value); ' +
    'document.getElementById(\'isospan\').innerHTML=this.value" ' +
    'style="width:200px;vertical-align:middle;display:inline;margin: ' +
    '0 10px 0"><span id="isospan">100</span>';
  ih += '<br/>';
  ih +=
    '<span>Time window (timestamps) : </span><input id="timeWindowRange" ' +
    'type="range" min="1" max="1000" value="' +
    data.zTimeSize +
    '" step="1" onmousedown="r3D.disableDrag(true)" ' +
    'onmouseup="r3D.disableDrag(false)" ' +
    'oninput="r3D.setTimeWindow(this.value,0,index)" ' +
    'style="width:200px;vertical-align:middle;display:inline;margin: ' +
    '0 10px 0"><input type="number"  id="timeWindowValue" ' +
    'onchange="r3D.setTimeWindow(this.value,1,index)" value="' +
    data.zTimeSize +
    '" style="width:45px;vertical-align:middle;display:inline">';
  ih +=
    '<br/><span>Delays (timestamps) :</span>' +
    '<span style="position:absolute;left:220px">Opacity :</span>';
  for (var i = 0; i < data.nLayers; i++) {
    ih += '<br/>';
    ih += '<span>' + data.layerNames[i] + '</span><input id="delayRange' + i +
      '" type="range" min="-5" max="5" value="0" step="1" ' +
      'onmousedown="r3D.disableDrag(true)" ' +
      'onmouseup="r3D.disableDrag(false)" ' +
      'oninput="r3D.setDelay(this.value,' + i +
      ')" style="width:100px;vertical-align:middle;display:inline;margin: ' +
      '0 10px 0;position:absolute;left:50px"><span style="position:absolute; ' +
      'left:180px" id="delayspan' + i + '">0</span>';
    ih += '<input id="opacityRange' + i +
      '" type="range" min="0" max="10" value="10" step="1" ' +
      'onmousedown="r3D.disableDrag(true)" ' +
      'onmouseup="r3D.disableDrag(false)" ' +
      'oninput="r3D.setOpacity(this.value,' + i +
      ')" style="width:100px;vertical-align:middle;display:inline; ' +
      'margin: 0 10px 0;position:absolute;left:210px">';
  };
...
function generate3Dparams() {
  ...
  var ih =
    '<button onclick="hideAll()">Hide all</button><button ' +
    'onclick="showAll()">Show all</button>';
  ih += '<ul>';
  ih +=
    '<li><input id="lfpCheckbox" type="checkbox" checked ' +
    'onchange="r3D.toggleLFPs(this.checked)">LFPs</li>';
  for (var i = 0; i < data.nLayers; i++) {
    ih += '<li><input id="' + 'popCheckBox' + i +
      '" type="checkbox" checked ' +
      'onchange="r3D.togglePop(this.checked,this.value)" value="' +
      i + '">' + data.layerNames[i] + '</li>';
  };
  ih +=
    '<li><input type="checkbox" ' +
      'onchange="r3D.toggleAxis(this.checked)">Axes</li>' +
    '<li><input type="checkbox" checked ' +
      'onchange="r3D.toggleTags(this.checked)">Name tags</li>' +
    '<li><input type="checkbox" checked ' +
      'onchange="r3D.toggleMarkers(this.checked)">Layer markers</li>' +
    '</ul>';

  if (data.dataType == "binned") {
    ih += '<ul><span>Use log scales : </span>' +
      '<li><input type="checkbox" onchange="r3D.toggleLogPos(this.checked)">' +
        'Position</li>' +
      '</ul>' + '<ul><span>Sum display settings : </span>' +
      '<li><input type="checkbox" ' +
      ...
      '<li><input id="sumScale" type="number" value="1" ' +
        ...
        'Background scale</li>' +
      '</ul>' + '<ul><span>Display part of the layers : </span>' +
      '<li>X from<input id="xMin" type="number" value="0" style="width:40px" ' +
        'onchange="r3D.setMinX(this.value)">to</li>' +
      '<li><input id="xMax" type="number" value="' + data.xNeurons +
      '" style="width:40px" onchange="r3D.setMaxX(this.value)"></li>' +
      '<li>Y from<input id="yMin" type="number" value="0" style="width:40px" ' +
        'onchange="r3D.setMinY(this.value)">to</li>' +
      '<li><input id="yMax" type="number" value="' + data.yNeurons +
      '" style="width:40px" onchange="r3D.setMaxY(this.value)"></li>' +
      '<li><button onclick="r3D.adjustDisplayedNeurons()">' +
        'Update view</li></ul>';
  } else if (data.dataType == "neuron") {
    ih +=
      '<ul><li><span>Display density </span><input ' +
        'onmousedown="r3D.disableDrag(true)"' +
        'onmouseup="r3D.disableDrag(false)" type="range" min="0" max="100" ' +
        'value="100" step="1" oninput="r3D.setDensity(this.value)" ' +
        'style="width:200px;vertical-align:middle;display:inline;margin: ' +
        '0 10px 0"></li>';
    ih +=
      '<li><input id="neuronDisplaySize" type="number" value="4" ' +
        'style="width:40px" onchange="r3D.setNeuronSize(this.value)">' +
        '</li></ul>';
  };
  ...
  ih = '<ul><span>Camera : </span><li></li>' +
    '<li><button id="topButton" onclick="r3D.moveCameraTop()"><span>' +
      'Top</span></button></li>' +
    '<li><button id="bottomButton" onclick="r3D.moveCameraBottom()"><span>' +
      'Bottom</span></button></li>' +
    '<li><button id="sideButton" onclick="r3D.moveCameraSide()"><span>' +
      'Side</span></button></li>' +
    '<li><button id="resetCameraButton" onclick="r3D.resetCamera()"><span>' +
      'Reset</span></button></li>' +
    '</ul>';
...
function generateGraphParams() {
  var ih = '<ul>';
  for (var i = 0; i < data.nLayers; i++) {
    ih += '<li><input id="graphPopCheck' + i +
      '" type="checkbox" checked ' +
      'onchange="graph.togglePop(this.checked,this.value)" value="' +
      i + '">' + data.layerNames[i] + '</li>';
  };
  ih += '</ul>';
...
function readFile(f, reader, n, file) {
  ...
  graph.computeData();
  tEnd = new Date();
  console.log("data processed in " + (tEnd.getTime() - tStart.getTime()) +
    " ms");
  document.getElementById('hn' + data.layerNames[n]).className = 'done';
  document.getElementById('sn' + data.layerNames[n]).innerHTML = "'" +
    file.name + "'";
  document.getElementById('hn' + data.layerNames[n]).ondragleave = function() {
    this.className = 'done';
    return false;
  };
...
function readLFPFile(reader, file) {
  ...
  document.getElementById('lfp').ondragleave = function() {
    this.className = 'done';
    return false;
  };
...
function readRawPosFile(reader, n, file) {
  ...
  document.getElementById('pos' + data.layerNames[n]).ondragleave = function() {
    this.className = 'done';
    return false;
  };
  document.getElementById('posn' + data.layerNames[n]).innerHTML = "'" + file
    .name + "'";
  console.log("data processed in " + (tEnd.getTime() - tStart.getTime()) +
    " ms");
}
...
function readerOnLoadCallback(reader, files, i, h) {
  ...
  console.log("Reading file of type " + fName[1] + ", " + (files.length - i) +
    "/" + files.length);
...
function init() {
  ...
  if (data.dataType == "neuron") {
    ...
    document.getElementById("sceneMenuItem").parentNode.style.display =
      "none";
  }
  ...
  var colorInner = "";
  for (var i = 0; i < data.nLayers; i++) {
    colorInner += "\<option value=\"" + i + "\">" + data.layerNames[i] +
      "\</option>";
  };
  ...
  r2D = new Visu.Renderer2D(document.getElementById('miniCanvasPanel'), data,
    "r2D");

  graph = new Visu.Graph(document.getElementById('graphCanvasPanel'), data);
  graph.drawLegend();

  corr = new Visu.Correlation(document.getElementById(
    'correlationCanvasPanel'), data);
...
function updateSelector(i) {
  document.querySelector("#colorPicker").color.fromString(data.layerColors[i]
    .substring(1));
};
...
function handleKeyDown(e) {
  if (e.shiftKey) {
    if (e.keyCode == r3D.keyCodes.s) {
      ...
      e.preventDefault();
      r3D.changeCameraControls(document.getElementById('cameraStyleButton'),
        1);
...
function barDown(e) {
  ...
  e.preventDefault();
  index = Math.floor((e.clientX - bar.getBoundingClientRect().left) / barW *
    data.timestamps);
  if (index < 0)
    index = 0;
...
function drawIndex() {
  ...
  barC.fillRect(Math.floor(((barW - 5) / (data.timestamps - 1)) * (index)), 0,
    5, barH);
...
function setupMenuItems() {
  ...
  for (var i = 0; i < subMenuItems.length; i++) {
    subMenuItems[i].addEventListener("mouseenter", function(e) {
      ...
      ul.style.top = (this.getBoundingClientRect().top -
...
function updateDataType() {
  ...
  var ph = document.getElementById("paramPlaceholder"),
    phe = document.getElementById("paramPlaceholderEnd"),
    newNodes = [];
  ...
  if (value == "binned") {
    ...
    newNodes[0].innerHTML =
      'Number of bins along x <input id="xNumber" type="number" value="40">';
    newNodes[1] = document.createElement("li");
    newNodes[1].innerHTML =
      'Number of bins along y <input id="yNumber" type="number" value="40">';
    ...
    newNodes[4].innerHTML =
      'Number of bins along x<input id="lfpx" type="number" value="10">';
    newNodes[5] = document.createElement("li");
    newNodes[5].innerHTML =
      'Number of bins along y<input id="lfpy" type="number" value="10">';
    ...
    newNodes[1] = document.createElement("li");
    newNodes[1].innerHTML =
      '<textarea id="posFiles" rows="1" cols="100"></textarea>';
  };
