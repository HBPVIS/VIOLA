...
Visu.Graph = function(panel, data) {
  ...
  //Number of miliseconds displayed on graph (actually 1 per pixel)
  this.spikesDataSize = this.svgW;
...
Visu.Graph.prototype = {
  ...
  setZoom: function(z, index) {
    ...
    document.getElementById("zoomValue").innerHTML =
      this.spikesDataSize * this.data.resolution;
  ...
  displayData: function(index) {
      ...
      for (var j = 0; j < this.data.nLayers; j++) {
        if (this.data.mReady[j] && this.displayPop[j]) {
          ...
          this.ctx.fillRect(Math.floor(i * this.eBWidth), offset,
                            this.bWidth,
                            this.svgH * this.spikesData[j][tOffset] /
                              this.total[tOffset]);
          offset += this.svgH * this.spikesData[j][tOffset] /
            this.total[tOffset];
        };
      };
    };
    ...
    this.ctx.moveTo(
      0,
      this.svgH - this.data.getScaledValue(this.total[0 + this.timeOffset]) *
        this.totalRatio);
    for (var i = 0; i < this.spikesDataSize; i++) {
      this.ctx.lineTo(i * this.eBWidth, this.svgH -
        this.data.getScaledValue(this.total[i + this.timeOffset]) *
        this.totalRatio);
    };
    this.ctx.lineTo(this.svgW,
                    this.svgH -
                      this.data.getScaledValue(this.total[this.spikesDataSize -
                        1 + this.timeOffset]) * this.totalRatio);
    this.ctx.stroke();
  ...
  drawIndexGraph: function(index) {
    ...
    this.ctx.fillStyle = "white";
    this.ctx.fillRect(Math.floor((index - this.timeOffset) * (this.svgW) /
                                 this.spikesDataSize) - 1,
                      0,
                      2,
                      this.svgH);
  },
  ...
  drawTimeAxe: function() {
    ...
    this.ctxT.fillText("Delay (ms)",
                       230 - this.ctxT.measureText("Delay (ms)").width / 2,
                       30);
    ...
    var maxNumTicks = 8;
    var realTicksSpacing =
      Math.ceil(this.spikesDataSize / (2 * maxNumTicks * roundSpacing)) *
        roundSpacing;
    ...
    for (var i = 1; i < numTicks + 1; i++) {
      ...
      this.ctxT.fillText("-" + i * realTicksSpacing * this.data.resolution,
                         230 - ticksSpacing * i - this.ctxT.measureText("-" +
                           i * realTicksSpacing * this.data.resolution).width /
                           2,
                         15);
      this.ctxT.fillRect(228 + ticksSpacing * i, 0, 2, 5);
      this.ctxT.fillText(i * realTicksSpacing * this.data.resolution,
                         229 + ticksSpacing * i - this.ctxT.measureText(i *
                           realTicksSpacing * this.data.resolution).width / 2,
                         15);
    };
  },
  ...
  drawAxes: function() {
    ...
    this.ctxA.fillText(sp[0] + unit[uToUse[0]],
                       13 - this.ctxA.measureText(sp[0] +
                                                  unit[uToUse[0]]).width / 2,
                       this.svgH / 2);
    this.ctxA.fillText(sp[1] + unit[uToUse[1]],
                       13 - this.ctxA.measureText(sp[1] +
                                                  unit[uToUse[1]]).width / 2,
                       5 * this.svgH / 8);
    this.ctxA.fillText(sp[2] + unit[uToUse[2]],
                       13 - this.ctxA.measureText(sp[2] +
                                                  unit[uToUse[2]]).width / 2,
                       3 * this.svgH / 4);
    this.ctxA.fillText(sp[3] + unit[uToUse[3]],
                       13 - this.ctxA.measureText(sp[3] +
                                                  unit[uToUse[3]]).width / 2,
                       7 * this.svgH / 8);
    this.ctxA.save();
