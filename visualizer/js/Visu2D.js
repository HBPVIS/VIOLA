...
Visu.Renderer2D = function(panel,data,name){
	...
	for(var i=0; i<data.nLayers; i++){
		...
		ih += '<td><canvas id="mini'+i+'" width="'+this.miniCW+'" height="'+this.miniCH+'"></canvas></td>';
	...
	ih += '<tr><td colspan=2><div style="margin:auto;margin-top:10px;width:310px">'
	...
	for(var c=0; c<data.nLayers; c++){
		this.mCtx[c] = document.getElementById("mini"+c).getContext("2d");
	}

	if(data.dataType == "binned")
		this.mCtxL = document.getElementById("miniL").getContext("2d");
...
Visu.Renderer2D.prototype = {
	...
	draw: function(index){
        ...
        var offset = [this.offsetW + this.miniW/2 - this.neuronDisplaySize / 2, this.offsetH + this.miniH/2 - this.neuronDisplaySize / 2];
        var scaling = [(this.miniW - this.neuronDisplaySize) / this.data.xSize, (this.miniH - this.neuronDisplaySize) / this.data.ySize];
			    ...
			    if(this.data.mReady[c]){
    				for(var i=0; i<this.data.xNeurons; i++){
    					for(var j=0; j<this.data.yNeurons; j++){
    						dataNow = this.data.getScaledValue(this.data.datasets[c][index][i*this.data.yNeurons+j]);
    						if(dataNow > 0){
    							this.mCtx[c].fillStyle = this.colorRect(dataNow);
    							this.mCtx[c].fillRect(this.offsetW+i*this.miniRectW,this.offsetH+j*this.miniRectH,this.miniRectW,this.miniRectH);
    						}
    					}
    				}
                }
			    ...
			    if(this.data.mReady[c] && this.data.posReady[c]){
                    ...
                    if(spikes !== undefined){
                        ...
                        for(var i=0; i < limit; i++){
                            displayPos = [this.data.neuronPos[spikes[i]][0],this.data.neuronPos[spikes[i]][1]];
                            ...
                            this.mCtx[c].fillRect(displayPos[0],displayPos[1], this.neuronDisplaySize, this.neuronDisplaySize);
                        }
                    }
                }
            }
	...
	drawMiniLegend: function (){
		...
		this.mCtxL.clearRect(0,0,this.miniLW,this.miniLH);
            ...
            var sp = [Math.round(this.data.getUnscaledValue(this.data.maxSpikesAmount/3)),
                Math.round(this.data.getUnscaledValue(2*this.data.maxSpikesAmount/3)),
                Math.round(this.data.getUnscaledValue(this.data.maxSpikesAmount))];
            var unit = ["","K","M","G","T"];
            var uToUse = [0,0,0];
    		...
    		this.mCtxL.fillText(sp[0]+unit[uToUse[0]],10-this.mCtxL.measureText(sp[0]+unit[uToUse[0]]).width/2+(this.miniLW-20)/3,12);
    		this.mCtxL.fillRect(10+(this.miniLW-20)/3,10,2,2);
    		this.mCtxL.fillText(sp[1]+unit[uToUse[1]],10-this.mCtxL.measureText(sp[1]+unit[uToUse[1]]).width/2+2*(this.miniLW-20)/3,12);
    		this.mCtxL.fillRect(10+2*(this.miniLW-20)/3,10,2,2);
    		this.mCtxL.fillText(sp[2]+unit[uToUse[2]],Math.min(this.miniLW-this.mCtxL.measureText(sp[2]+unit[uToUse[2]]).width,this.miniLW-10-this.mCtxL.measureText(sp[2]+unit[uToUse[2]]).width/2),12);
	...
	drawMiniLegends: function (){
		for(var c=0; c<this.data.nLayers; c++){
			...
			this.mCtx[c].fillStyle="black";
			this.mCtx[c].fillText(data.layerNames[c], (this.miniCW-this.mCtx[c].measureText(this.data.layerNames[c]).width)/2, 2);
			this.mCtx[c].font = "12px serif";
			this.mCtx[c].textBaseline = "middle";
			this.mCtx[c].fillText(-this.data.ySize/2,this.offsetW-2-this.mCtx[c].measureText(-this.data.ySize/2).width,this.offsetH);
			this.mCtx[c].fillText(-this.data.ySize/4,this.offsetW-2-this.mCtx[c].measureText(-this.data.ySize/4).width,this.offsetH+this.miniH/4);
			this.mCtx[c].fillText("0",this.offsetW-2-this.mCtx[c].measureText("0").width,this.offsetH+this.miniH/2);
			this.mCtx[c].fillText(this.data.ySize/4,this.offsetW-2-this.mCtx[c].measureText(this.data.ySize/4).width,this.offsetH+3*this.miniH/4);
			this.mCtx[c].fillText(this.data.ySize/2,this.offsetW-2-this.mCtx[c].measureText(this.data.ySize/2).width,this.offsetH+this.miniH);
			this.mCtx[c].textBaseline = "top";
			this.mCtx[c].fillText(this.data.xSize/2,this.offsetW-this.mCtx[c].measureText(this.data.xSize/2).width/2+this.miniW,this.offsetH+this.miniH+2);
			this.mCtx[c].fillText(this.data.xSize/4,this.offsetW-this.mCtx[c].measureText(this.data.xSize/4).width/2+3*this.miniW/4,this.offsetH+this.miniH+2);
			this.mCtx[c].fillText("0",this.offsetW-this.mCtx[c].measureText("0").width/2+this.miniW/2,this.offsetH+this.miniH+2);
